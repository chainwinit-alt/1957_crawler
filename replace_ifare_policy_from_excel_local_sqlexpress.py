...
# ===== 可調整設定：依寫入目標切換這一區即可 =====
# 本機版預設給遠端 Windows 主機「直接在主機內執行」使用。
# 目前該台 SQLEXPRESS 尚未啟用 TCP listener，因此預設改走
# localhost\SQLEXPRESS 的本機連線模式，不依賴對外 port。
ACTIVE_DB_TARGET = "local_sqlexpress_trusted"

DB_TARGETS: dict[str, dict[str, Any]] = {
    "local_sqlexpress_trusted": {
    ...
    # 若這台 Windows 帳號沒有 IFare DB 權限，可改填 SQL 帳密後，
    # 再把 ACTIVE_DB_TARGET 切到 local_sqlexpress_sql_auth。
    "local_sqlexpress_sql_auth": {
        "server": r"localhost\SQLEXPRESS",
        "database": "IFare",
        "auth_mode": "sql",
        "username": None,
...
if ACTIVE_DB_TARGET not in DB_TARGETS:
    raise RuntimeError(
        f"ACTIVE_DB_TARGET={ACTIVE_DB_TARGET!r} 不存在，可用目標：{sorted(DB_TARGETS)}"
    )

ACTIVE_DB_CONFIG = DB_TARGETS[ACTIVE_DB_TARGET]
SERVER = str(ACTIVE_DB_CONFIG["server"])
DATABASE = str(ACTIVE_DB_CONFIG["database"])
...
def latest_excel_on_desktop() -> Path:
    """優先找腳本同目錄，其次找桌面上最新的 1957 爬蟲 Excel。"""

    candidates: list[Path] = []
    seen: set[Path] = set()
    for folder in (SCRIPT_DIR, Path.home() / "Desktop"):
        for path in folder.glob("1957政策_*.xlsx"):
    ...
    if not candidates:
        raise FileNotFoundError(
            f"找不到 1957政策_*.xlsx；已搜尋 {SCRIPT_DIR} 與 {Path.home() / 'Desktop'}"
        )
    return candidates[0]
...
def connect(*, autocommit: bool) -> pyodbc.Connection:
    ...
    auth_mode = str(ACTIVE_DB_CONFIG.get("auth_mode", "trusted")).lower()
    encrypt = str(ACTIVE_DB_CONFIG.get("encrypt", "no"))
    trust_server_certificate = str(ACTIVE_DB_CONFIG.get("trust_server_certificate", "yes"))
    ...
    # 舊版 SQL Server ODBC driver 不支援 Encrypt / TrustServerCertificate 參數，
    # 若硬塞進去反而會直接報「無效的連線字串屬性」。
    ...
    if auth_mode == "trusted":
        ...
        username = ACTIVE_DB_CONFIG.get("username")
        password = ACTIVE_DB_CONFIG.get("password")
        if not username or password is None:
            raise RuntimeError(f"{ACTIVE_DB_TARGET} 缺少 SQL 帳號或密碼設定")
        conn_parts.append(f"UID={username}")
        conn_parts.append(f"PWD={password}")
    else:
        raise RuntimeError(f"不支援的 auth_mode={auth_mode!r}，只接受 trusted / sql")
    ...
    try:
        ...
        message = str(exc)
        if driver == "SQL Server" and ("SSL 安全設定錯誤" in message or "SECCreateCredentials" in message):
            raise RuntimeError(
                "目前這台電腦只安裝到舊版 ODBC driver「SQL Server」，"
                "已可連到目標 SQL Server，但在 SSL/TLS 交握時失敗。"
                "請先安裝 Microsoft ODBC Driver 17 或 18 for SQL Server，"
                "安裝後本程式會自動優先使用新版 driver。"
            ) from exc
        raise
...
def main() -> None:
    ...
    print(f"DB 目標：{ACTIVE_DB_TARGET} -> {SERVER} / {DATABASE}")
